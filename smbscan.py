...
def printStatus(target, color, message, options):
	...
	if target is None:
		...
		status = "%1s%2s %3s (%4s): %5s" %(color, statusSymbol, target.ip, target.name, message)
...
def listFiles(target, smbClient, share, sharePath, options, logFile, currentDepth):
	try:
		for f in smbClient.listPath(share.shareName, sharePath + '\\*'):
			...
			keywords = ["password", "web.config", "wp-config.php", "passport", "handover"]
			if any(keyword in sharedFile.fileName for keyword in keywords):
				printStatus(target, Colors.OKBLUE, "%crw-rw-rw- %10d  %s %s" % ('d' if f.is_directory() > 0 else '-', f.get_filesize(), time.ctime(float(f.get_atime_epoch())), f.get_longname()), options)
			...
			if f.is_directory() > 0 and (options.maximumDepth == 0 or currentDepth < options.maximumDepth):
				listFiles(target, smbClient, share, sharedFile.fullPath, options, logFile, currentDepth+1)
	except Exception as e:
		printStatus(target, Colors.FAIL, "ERROR ACCESSING %1s: %3s" %(share.shareName, e), options)
		return
	finally:
		if options.jitter > 0:
...
def getFiles(smbClient, target, options, logFile):
	for share in target.shares:
		if share.shareName in options.exclusionList:
			printStatus(target, Colors.WARNING, "Skipping %1s (on exclusion list)" %(share.shareName), options)
		elif len(options.inclusionList) > 0:
			if share.shareName in options.inclusionList:
				printStatus(target, Colors.OKBLUE, "Scanning %1s (on inclusion list)" %(share.shareName), options)
				listFiles(target, smbClient, share, '', options, logFile, 1)
			else:
				printStatus(target, Colors.WARNING, "Skipping item %1s (not on inclusion list)" %(share.shareName), options)
		else:
			printStatus(target, Colors.OKBLUE, "Scanning %1s" %(share.shareName), options)
			listFiles(target, smbClient, share, '', options, logFile, 1)

def getClient(target, user, options, port):
	try:
		smbClient = SMBConnection('*\\*SMBSERVER*', target.ip, timeout=options.timeout, sess_port=int(port))
		if options.kerberos is True:
			smbClient.kerberosLogin(user.username, user.password, user.domain, user.lmhash, user.nthash, options.aesKey, options.dc_ip )
		else:
			...
			smbClient.login(user.username, user.password, user.domain, user.lmhash, user.nthash, ntlmFallback=True)
		#Host is live
		return smbClient
	except SessionError as e:
		printStatus(target, Colors.WARNING, "SESSION FAILURE: (%1s, %2s) %3s" %(user.username, str(port), str(e)), options)
		#Host is live
		return None
	except Exception as e:
		if "timed out" in str(e):
			...
			printStatus(target, Colors.WARNING, "CONNECTION FAILURE: (%1s, %2s) %3s" %(user.username, str(port), str(e)), options)
			#Host is live
		else:
			printStatus(target, Colors.FAIL, "CONNECTION FAILURE: (%1s, %2s) %3s" %(user.username, str(port), str(e)), options)
			#Host is live
		return None
...
def scanSingle(targetHost, user, options):
	...
	if (smbClient != None):
		try:
			...
			logFileName = options.logFileName if options.logFileName else "logs/smbscan-"+slugify(target.name)+"-"+fileTimeStamp+".csv"
			logFile = open(logFileName, "a")

			printStatus(target, Colors.OKGREEN, "CONNECTED AS %1s - %2s" %(user.username, smbClient.getServerOS()), options)
...
def main():
	...
	group.add_argument("target", nargs="?", help="Target for scanning. e.g. 192.168.1.1 or 192.168.1.0/24")
	group.add_argument("-f", "--file", help="List of targets for scanning.", type=argparse.FileType('r'))
	parser.add_argument("-u", "--user", help="User to connect as")
	parser.add_argument("-p", "--password", help="Password for user (will prompt if missing and needed)")
	parser.add_argument("-d", "--domain", help="Domain for user")
	parser.add_argument("-k", "--kerberos", help="Not implemented", action="store_true")
	parser.add_argument("-j", "--jitter", help="Random delay between some requests. Default 3 seconds.", type=int, default=3)
	parser.add_argument("-t", "--timeout", help="Set timeout for connections. Default 2 seconds.", type=int, default=2)
	parser.add_argument("-s", "--shares", help="Display shares only - dont crawl files", action="store_true")
	parser.add_argument("-l", "--logFileName", help="Override log file name (without extension)")
	parser.add_argument("-i", "--inclusionList", help="List of comma separated shares to include in scan. All others will be excluded.")
	parser.add_argument("-e", "--exclusionList", help="List of comma separated shares to exclude from scan. All others will be included.")
	parser.add_argument("-m", "--maximumDepth", help="Maximum depth to crawl. 0 (default) = unlimited.", type=int, default=0)
	...
	options.outputLogFileName = "logs/smbscan-" + time.strftime("%Y%m%d-%H%M%S") + ".txt"
	options.crawlShares = not args.shares
	options.maximumDepth = args.maximumDepth
	if str(args.inclusionList) != 'None': options.inclusionList = str(args.inclusionList).split(",")
	if str(args.exclusionList) != 'None': options.exclusionList = str(args.exclusionList).split(",")
